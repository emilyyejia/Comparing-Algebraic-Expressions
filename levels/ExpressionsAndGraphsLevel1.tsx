...
const ExpressionsAndGraphsLevel1: React.FC<LevelComponentProps> = ({ 
  ...
  const handleDotClick = (pointIndex: number) => {
    ...
    const selectedPoint = linearPoints[selectedPairIndex];
    const clickedPoint = linearPoints[pointIndex];
    ...
    if (selectedPoint.x === clickedPoint.x && selectedPoint.y === clickedPoint.y) {
      setPlacedIndices(prev => {
        ...
        if (next.size === linearPoints.length) {
  ...
  const handleCheckAnswers = () => {
    ...
    if (allOk) {
      ...
      setErrorCount(prev => prev + 1);
      setFeedback("Try again! What is its x‑value? What is its y‑value? Write them as (x, y).");
    }
  };
  ...
  return (
    <div className="flex flex-col items-center min-h-full p-8 bg-slate-950 font-sans max-w-7xl mx-auto pb-24 text-xl">
      ...
      >
        {phase === 'plotting' ? (
          <p>Plot y = 2x by clicking ordered pairs to their matching points on the grid.</p>
        ) : (
          <p>Identify the coordinates of the points on the graph of y = 2x² {constantC >= 0 ? '+ ' : '– '}{Math.abs(constantC)}.</p>
        )}
      </InstructionModal>
      ...
      <div className="w-full text-center py-6 mb-8 border-b border-slate-800">
         <h2 className="text-2xl md:text-3xl font-medium text-slate-200">
           {phase === 'plotting' 
             ? <span>Plot y = 2x by clicking ordered pairs to their matching points on the grid.</span>
             : <span>Identify the coordinates of the points on the graph of <span className="text-fuchsia-400 italic">y = 2x² {constantC >= 0 ? '+ ' : '– '}{Math.abs(constantC)}</span></span>}
         </h2>
      </div>
        ...
        <div className="flex flex-col w-full lg:w-[350px] gap-6">
          <div className="bg-slate-900 p-8 rounded-[2rem] border border-slate-800 shadow-2xl space-y-6">
            <h2 className="text-xl font-bold text-sky-300 uppercase tracking-widest border-b border-slate-800 pb-4">
              {phase === 'plotting' ? 'Ordered Pairs' : 'Coordinates of the Points'}
            </h2>
            
            {phase === 'plotting' ? (
              <div className="space-y-3">
                {linearPoints.map((p, idx) => {
        ...
        <div className="flex flex-col items-center">
          <div className="bg-white rounded-[2.5rem] p-12 shadow-2xl relative border-[12px] border-slate-900 group">
            ...
            >
              ...
              {phase === 'plotting' && linearPoints.map((pt, i) => {
              ...
              {placedIndices.size === linearPoints.length && phase === 'plotting' && (
                <line 
